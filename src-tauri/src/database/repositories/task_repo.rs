...
pub async fn find_by_id(pool: &Pool<Sqlite>, id: &str) -> Result<Task, sqlx::Error> {
    sqlx::query_as::<_, Task>("SELECT * FROM task_tracker WHERE id = ?")
        .bind(id)
        .fetch_one(pool)
        .await
}
...
pub async fn set_favorite_status(
    ...
    sqlx::query("UPDATE task_tracker SET favorite = ? WHERE id = ?")
        .bind(is_favorite)
        .bind(id)
        .execute(pool)
        .await?;
...
pub async fn find_all_raw(pool: &Pool<Sqlite>) -> Result<Vec<Task>, sqlx::Error> {
    sqlx::query_as::<_, Task>("SELECT * FROM task_tracker")
        .fetch_all(pool)
        .await
}
...
pub async fn reset_task_progress(
    ...
    let res =
        sqlx::query("UPDATE task_tracker SET current_completions = 0, last_reset = ? WHERE id = ?")
            .bind(reset_time)
            .bind(id)
            .execute(pool)
            .await?;
