...
pub async fn set_task(state: State<'_, UserDb>, id: String, count: i32) -> Result<Task, String> {
    debug!("set_task called | ID: {} | Count: {}", id, count);

    let pool = state.0.lock().await;

    let task = task_repo::find_by_id(&*pool, &id).await.map_err(|e| {
        let err = format!("Failed to find task '{}': {}", id, e);
        error!("{}", err);
        err
    })?;
    ...
    let updated_task = task_repo
        ...
        .map_err(|e| {
            let err = format!("Failed to update completions for task '{}': {}", id, e);
            error!("{}", err);
            err
        })?;

    info!(
        "Successfully updated task '{}' | New Count: {} | Reset Time: {}",
        id,
        count,
        final_reset_time
    );
